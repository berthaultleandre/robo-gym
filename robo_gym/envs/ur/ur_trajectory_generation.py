...
class TrajectoryGenerationUR(URBaseEnv):
    ...
    def env_action_to_rs_action(self, action) -> np.array:
        ...
        #world_name:=my_end_effector_positioning_with_camera.world \
class TrajectoryGenerationURSim(TrajectoryGenerationUR, Simulation):
    ...
    def __init__(self, ip=None, lower_bound_port=None, upper_bound_port=None, gui=False, ur_model='ur5', **kwargs): #camera=False
        self.cmd = self.cmd + ' ' + 'ur_model:=' + ur_model # + ' ' + 'camera:=' + str(camera)
        Simulation.__init__(self, self.cmd, ip, lower_bound_port, upper_bound_port, gui, **kwargs)
        TrajectoryGenerationUR.__init__(self, rs_address=self.robot_server_ip, ur_model=ur_model, **kwargs)

class TrajectoryGenerationURRob(TrajectoryGenerationUR):
    real_robot = True
